...
export default function List({ roles, auth, filters }) {
    ...
    const openDesignationsModal = (designations, departmentName) => {
        setSelectedDesignations(designations);
        setSelectedDepartmentName(departmentName);
        setShowDesignationsModal(true);
    };
    ...
    const updateUrl = (newPage = 1) => {
        const params = {
            ...
            page: newPage,
                 ...
                 setIsLoading(true);
        router.get(route("super.departments"), params, {
            preserveState: true,
            replace: true,
            preserveScroll: true,
    ...
    const handleSubmit = (e) => {
        ...
        const endpoint = currentRole
            ? route("super.update.departments", currentRole.uuid)
            : route("super.add.departments");
    ...
    const handleDelete = async () => {
        try {
            await router.delete(
                route("super.department.destroy", departmentToDelete),
                {},
                {
                    preserveScroll: true,
                    onSuccess: () => {
    ...
    const handleStatusUpdate = async () => {
        try {
            await router.post(
                route("super.status.departments", roleToUpdate),
                { status: newStatus },
                {
                    preserveScroll: true,
                    onSuccess: () => {
                    ...
                    >
                        <ul>
                            ...
                            <li className="flex items-center gap-[5px] p-2 text-[12px] text-black hover:bg-gray-100 cursor-pointer">
                                <button
                                    onClick={() => {
                                        setDepartmentToDelete(taskItem.uuid);
                            ...
                            <table className="min-w-full text-black rounded-2xl dark:text-white">
                                <thead>
                                    <tr className="whitespace-nowrap text-left">
                                        ...
                                        <th className="p-3">Designations</th>
                                                                ...
                                                                .map(
                                                                    (
                                                                        ...
                                                                        <button
                                                                            ...
                                                                            className="text-blue-600 dark:text-blue-400 hover:underline px-1 cursor-pointer"
                                                                            key={
                                                                                designation.id
                                                                            }
                                                                        >
    ...
    <div className="mt-4 flex justify-between items-center flex-wrap gap-4" style={{ padding: '0px 34px' }}>
        ...
        <nav aria-label="Pagination" className="flex items-center gap-2">
            ...
            <ConfirmDialog
                ...
                modalSpinnerMessage="Deleting department..."
                isDanger={true}
            />
            <ConfirmDialog
                ...
                cancelText="No, cancel"
                modalSpinnerMessage="Updating Department status..."
            />

            <Modal
                show={showDesignationsModal}
                onClose={() => setShowDesignationsModal(false)}
            >
                <div className="p-6 dark:bg-[#0a0e25]">
                    <h2 className="text-lg font-medium text-gray-900 dark:text-white mb-4">
                        Designations in {selectedDepartmentName}
                    </h2>
                    <div className="flex flex-wrap gap-2">
                        {selectedDesignations.map((designation) => (
                            <a
                                href={route("super.designation.list", {
                                    search: designation.slug,
                                })}
            ...
            >
                <h2 className="text-xl font-bold mb-4 dark:text-white">
                    {currentRole ? "Edit Departments" : "Create Departments"}
                </h2>
