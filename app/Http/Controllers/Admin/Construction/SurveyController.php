...
class SurveyController extends Controller
{
    ...
    public function index(
        ConstructionAuthorizationService $authorizationService
    ): Response {
        ...
        $permissionsByProject = $assignedProjectIds->mapWithKeys(
            fn (int $projectId) => [
                $projectId => $authorizationService->permissionsFor(
                    $actor,
                    $projectId
                ),
            ]
        ...
        $hasProjectPermission = fn (
            int $projectId,
            string $permission
        ): bool => in_array(
            $permission,
            $permissionsByProject->get($projectId, []),
            true
        );
        ...
        $planManageProjectIds = $projectIds
            ->filter(
                fn (int $projectId) =>
                    $hasProjectPermission(
                        $projectId,
        ...
        $teamMemberships = ProjectTeamMember::query()
            ->whereIn('project_id', $planManageProjectIds)
            ->where('status', 'active')
            ->get([
                'project_id',
                'member_id',
    ...
    private function adminActor(): Member
    {
        $actor =
            $this->constructionActor();

        abort_unless(
            $actor instanceof Member,
            403,
            'Unauthorized.'
        );
